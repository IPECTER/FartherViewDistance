...
public final class Chunk implements BranchChunk {
    ...
    public Chunk(WorldServer worldServer, IChunkAccess chunkAccess) {
    ...
    public org.bukkit.Chunk getChunk() {
        ...
        if (chunk instanceof ChunkEmpty) return ((ChunkEmpty) chunk).bukkitChunk;
        else if (chunk instanceof net.minecraft.world.level.chunk.Chunk) return ((net.minecraft.world.level.chunk.Chunk) chunk).bukkitChunk;
        else if (chunk instanceof ProtoChunk) return new net.minecraft.world.level.chunk.Chunk(worldServer, ((ProtoChunk) chunk), v -> {
        }).getBukkitChunk();
        else return null;
    }
    ...
    public void setIBlockData(int x, int y, int z, IBlockData iBlockData) {
        ...
        if (indexY >= 0 && indexY < chunkSections.length) {
            ChunkSection chunkSection = chunkSections[indexY];
            if (chunkSection == null) chunkSection = chunkSections[indexY] = new ChunkSection(indexY);
