...
/**
 * @see net.minecraft.world.level.chunk.storage.ChunkRegionLoader
 */
public final class ChunkRegionLoader {
    ...
    public static BranchChunk.Status loadStatus(NBTTagCompound chunkNBT) {
        ...
        return Chunk.ofStatus(ChunkStatus.a(levelNBT.getString("Status")));
...
//            LOGGER.error("Chunk file at {} is in the wrong location; relocating. (Expected {}, got {})", chunkLocation, chunkLocation, locationNBT);
        ...
        for (int index = 0; index < sectionsListNBT.size(); ++index) {
            ...
            if (sectionNBT.hasKeyOfType("Palette", 9) && sectionNBT.hasKeyOfType("BlockStates", 12)) {
                ...
                Chunk.recalculateBlockCounts(section);
                if (!section.c()) {
        ...
        if (statusType == ChunkStatus.Type.b) {
            ...
            net.minecraft.world.level.chunk.Chunk chunk = new net.minecraft.world.level.chunk.Chunk(worldServer.getMinecraftWorld(), chunkLocation, biomeStorage, chunkConverter, tickListBlock, tickListFluid, inhabitedTime, sections, (asyncChunk) -> {
            });
            chunkAccess = chunk;
        ...
        return new Chunk(worldServer, chunkAccess);
    }
    ...
    public static BranchChunkLight loadLight(WorldServer worldServer, NBTTagCompound chunkNBT) {
        ...
        ChunkLight chunkLight = new ChunkLight(worldServer);
    ...
    public static NBTTagCompound saveChunk(WorldServer worldserver, IChunkAccess chunkAccess, ChunkLight chunkLight, List<Runnable> asyncRunnable) {
        ...
        for (int index = worldserver.getMinSection() - 1; index < worldserver.getMinSection() - 1 + worldserver.getSectionsCount() + 2; ++index) {
            int sectionY = index;
            ChunkSection section = Arrays.stream(sections).filter((chunkSection) -> chunkSection != null && chunkSection.getYPosition() >> 4 == sectionY).findFirst().orElse(net.minecraft.world.level.chunk.Chunk.a);
            ...
            if (section != net.minecraft.world.level.chunk.Chunk.a || blockLight != null || skyLight != null) {
                NBTTagCompound sectionNBT = new NBTTagCompound();
                sectionNBT.setByte("Y", (byte) (sectionY & 255));
                if (section != net.minecraft.world.level.chunk.Chunk.a)
